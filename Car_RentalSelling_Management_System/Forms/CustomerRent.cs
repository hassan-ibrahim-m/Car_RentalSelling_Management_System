...
{
    public partial class Customer_Rent : Form
    {
        User user = null;
        List<Cars> cars = null;
        ...
        public Customer_Rent(User user)
        {
            this.user = user;
            ...
            InitializeComponent();
            db_City.Items.Add("Morrisville");
            db_City.Items.Add("Fairfax");
            db_City.Items.Add("Willowbrook");
            db_City.Items.Add("San Jose");
            db_City.Items.Add("Richmond");
            db_City.Items.Add("Houston");
        }
        ...
        private void Customer_Rent_Load(object sender, EventArgs e)
        {
        ...
        private void bt_submit_Click(object sender, EventArgs e)
        {
            ...
            if (db_CarModel.SelectedIndex != -1
                && db_Manufacturer.SelectedIndex != -1
                && db_City.SelectedIndex != -1
                && db_type.SelectedIndex != -1
                && db_MaxCost.SelectedIndex != -1
                && db_MaxYear.SelectedIndex != -1
                && db_MinCost.SelectedIndex != -1
                && db_MinYear.SelectedIndex != -1
                )
            {
                string Manufacturer = db_Manufacturer.SelectedItem.ToString();
                string City = db_City.SelectedItem.ToString();
                string Type = db_type.SelectedItem.ToString();
                string CarModel = db_CarModel.SelectedItem.ToString();
                string MaxCost = db_MaxCost.SelectedItem.ToString();
                string MinCost = db_MinCost.SelectedItem.ToString();
                string MinYear = db_MinYear.SelectedItem.ToString();
                string MaxYear = db_MaxYear.SelectedItem.ToString();

                MessageBox.Show("Your reservation established successfuly!");
            }
            else
            {
                MessageBox.Show("Please complete your filters!");
            }
        ...
        private void LVRentRec_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        {
            string Manufacturer = db_Manufacturer.Text;
            string CarModel = db_CarModel.Text;
            string MinYear = db_MinYear.Text;
            string MaxYear = db_MaxYear.Text;
            string Type = db_type.Text;
            string City = db_City.Text;
            string MinPrice = db_MinCost.Text;
            string MaxPrice = db_MaxCost.Text;

            if (db_Manufacturer.SelectedIndex == -1 || db_CarModel.SelectedIndex == -1
                || db_MinYear.SelectedIndex == -1 || db_MaxYear.SelectedIndex == -1
                || db_type.SelectedIndex == -1 || db_City.SelectedIndex == -1
                || db_MinCost.SelectedIndex == -1 || db_MaxCost.SelectedIndex == -1)
            {
                MessageBox.Show("Pleas complete your filter");
            }
            else
            {
                Connection.OpenConection();
                cars = Connection.SelectCars_rent(Manufacturer, CarModel, MinYear, MaxYear, MinPrice, MaxPrice, Type, City);
                lv_rent.Items.Clear();
                foreach (Cars c in cars)
                {
                    ListViewItem lv = new ListViewItem(new string[] { c.Manufacturer, c.Rate.ToString(), c.CarModel, c.Year.ToString(), c.Price.ToString(), c.KM.ToString(), c.City, c.Price.ToString(), c.FuelType, c.Type });
                    lv_rent.Items.Add(lv);
                }
        ...
        {
            CustomerHome c = new CustomerHome(user);
            c.Show();
            Hide();
        }

        private void db_Manufacturer_SelectedIndexChanged_1(object sender, EventArgs e)
        {
            db_CarModel.Items.Clear();
            foreach (string element in dic[db_Manufacturer.SelectedItem.ToString()])
                db_CarModel.Items.Add(element);
        ...
        private void db_City_SelectedIndexChanged(object sender, EventArgs e)
        {

        }

        private void lv_rent_SelectedIndexChanged(object sender, EventArgs e)
        {
